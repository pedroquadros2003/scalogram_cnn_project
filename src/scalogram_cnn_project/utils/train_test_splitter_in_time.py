...
from scalogram_cnn_project.utils.generic_operations_list_of_numpy import sort_X, slice_X, index_X, concat_X, get_num_epochs
...
def train_test_split_aux(X, y, test_size, random_state, overlap, neglected_epochs_step):
    ...
    threshold_epochs = int(get_num_epochs(X) * (1 - test_size))
    ...
    logger.debug("x_train : %s", x_train[0].shape if isinstance(x_train, list) else x_train.shape)
    logger.debug("x_test  : %s", x_test[0].shape if isinstance(x_test, list) else x_test.shape)
...
def train_test_split(X, y, test_size, random_state, overlap, subject_array, epoch_array, neglected_epochs_step=1):
    ...
    x_train = concat_X(x_train_list)
    x_test  = concat_X(x_test_list)
