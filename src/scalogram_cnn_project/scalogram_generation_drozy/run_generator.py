...
def run_generation(config, mode):
    ...
    if mode == "simple":
        from scalogram_cnn_project.scalogram_generation_drozy.generator_scalogram_simple import generate_scalogram
        ...
        generate_scalogram(
            ...
            do_resampling=params.get("do_resampling"),
            resample_freq=params.get("resample_freq"),
            drowsiness_threshold=params.get("drowsiness_threshold"),
        ...
        if extra_input:
            ...
            for subject in subjects:
                for session in sessions:
                    if not project_config.drozy_valid_tests[subject][session]:
                        continue
                    
                    for ch_idx, channel in enumerate(channels, start=1):
                        logger.info(f"Generating DROZY {subject}-{session} | {channel}")
            ...
            from scalogram_cnn_project.scalogram_generation_drozy.generator_scalogram_batch import generate_scalogram
